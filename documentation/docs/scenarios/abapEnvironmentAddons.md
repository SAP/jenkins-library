...
In case of an error from a backend system during execeution of the pipeline steps:

| Stage              | Step                            | Issue                                                                                     | Resolution                                                                                                                                                                                                                        |
|--------------------|---------------------------------|-------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Prepare System     | abapEnvironmentCreateSystem     | Add-on assembly system cannot be created due to missing entitlements                      | System provisioning requires entitlements for abap/standard as well as abap/hana_compute_unit and abap/abap_compute_unit to be assigned to the subaccount.                                                                        |
| Clone Repositories | abapEnvironmentPullGitRepo      | Commit ID cannot be found in branch                                                       | Branches may include different commits, make sure the commit is in the commit history of the branch.                                                                                                                              |
|                    | abapEnvironmentPullGitRepo      | Timeout                                                                                   | Parallel execution of multiple actions on the same software component (like checkout, pull etc.) is not supported.                                                                                                                |
| ATC                | abapEnvironmentRunATCCheck      | Long running step execution                                                               | Use custom check variants and exemptions to reduce the test scope and irrelevant findings.                                                                                                                                        |
| Build              | abapEnvironmentAssemblePackages | New software component is not included during add-on build                                | New Software Component added, but same add-on product release: New software components can only be introduced as part of a new add-on product release version.                                                                    |
|                    | abapEnvironmentAssemblePackages | Application content does not change with new add-on version                               | Add-on Product Version changed, but software component versions remain the same: New delivery packages for software components are only created if a new version of a software component is defined.                              |
|                    | abapEnvironmentAssemblePackages | Application content does not change with commitID of software component                   | New commitID, but no new software component version: There should be a warning in this case, that only by changing the version a new delivery package is created.                                                                 |
|                    | abapEnvironmentAssemblePackages | Correction Package: Previous Commit ID cannot be found                                    | New Patch Level, branch is changed: There is a potential here that the commit id stored for a previously created correction package is not available in another branch.                                                           |
|                    | abapEnvironmentAssemblePackages | Correction Package: Previous Commit ID = Current Commit ID                                | New Patch Level, same commitID: The same commitID cannot be used as previous/current commit id for a correction package.                                                                                                          |
| Integration Tests  | abapEnvironmentCreateSystem     | Add-on installation test system is not created due to missing entitlements                | System provisioning requires entitlements for abap/saas_oem as well as abap/hana_compute_unit and abap/abap_compute_unit to be assigned to the subaccount.                                                                        |
|                    | abapEnvironmentCreateSystem     | Add-on installation test system is not created because it still exists                    | If the add-on installation system still exists from a previous add-on build, the add-on installation test cannot be performed using the existing system.                                                                          |
|                    | abapEnvironmentCreateSystem     | Add-on installation test system is not created because of missing add-on registration     | If the add-on installation system still exists from a previous add-on build, the add-on installation test cannot be performed using the existing system.                                                                          |
| Post               | cloudFoundryDeleteService       | Add-on assembly system is deleted unexpectedly                                            | Depending on confirmDeletion parameter as well as manual confirmation, the add-on assembly is deleted after every pipeline execution. A pipeline configuration with permanent add-on assembly system can be found in the example. |

Once execution of the `Build` stage, in particular the `abapAddonAssemblyKitRegisterPackages` step, has been completed, errors must be resolved by creating new software component versions to cause another local build.
Pipeline steps can be reexecuted without causing double execution of already perfomed steps (intermediate results are stored).


If these steps do no resolve the issue, please open a [support incident](https://launchpad.support.sap.com/#/notes/1296527) on the respective support component:

...
type packBuildOptions struct {
	...
	PackDownloadURL      string                 `json:"packDownloadUrl,omitempty"`
...
type packBuildCommonPipelineEnvironment struct {
	container struct {
		registryURL   string
		imageDigest   string
		imageNameTag  string
...
func (p *packBuildCommonPipelineEnvironment) persist(path, resourceName string) {
	content := []struct {
		category string
		name     string
		value    interface{}
...
type packBuildReports struct {
}

func (p *packBuildReports) persist(stepConfig packBuildOptions, gcpJsonKeyFilePath string, gcsBucketId string, gcsFolderPath string, gcsSubFolder string) {
	if gcsBucketId == "" {
...
// PackBuildCommand Executes pack cli
func PackBuildCommand() *cobra.Command {
	const STEP_NAME = "packBuild"

	metadata := packBuildMetadata()
	var stepConfig packBuildOptions
	var startTime time.Time
	var commonPipelineEnvironment packBuildCommonPipelineEnvironment
	var reports packBuildReports
	var logCollector *log.CollectorHook
	var splunkClient *splunk.Splunk
	telemetryClient := &telemetry.Telemetry{}

	var createPackBuildCmd = &cobra.Command{
		Use:   STEP_NAME,
		Short: "Executes pack cli",
		Long:  `Runs ` + "`" + `pack build` + "`" + ` command`,
		...
		Run: func(_ *cobra.Command, _ []string) {
			...
			packBuild(stepConfig, &stepTelemetryData, &commonPipelineEnvironment)
			stepTelemetryData.ErrorCode = "0"
			log.Entry().Info("SUCCESS")
		},
	}

	addPackBuildFlags(createPackBuildCmd, &stepConfig)
	return createPackBuildCmd
}

func addPackBuildFlags(cmd *cobra.Command, stepConfig *packBuildOptions) {
	cmd.Flags().StringVar(&stepConfig.Builder, "builder", `paketobuildpacks/builder-jammy-base:latest`, "Builder")
	...
	cmd.Flags().StringVar(&stepConfig.SyftDownloadURL, "syftDownloadUrl", `https://github.com/anchore/syft/releases/download/v1.3.0/syft_1.3.0_darwin_amd64.tar.gz`, "Specifies the download url of the Syft Linux amd64 tar binary file. This can be found at https://github.com/anchore/syft/releases/.")
	cmd.Flags().StringVar(&stepConfig.PackDownloadURL, "packDownloadUrl", `https://github.com/buildpacks/pack/releases/download/v0.33.2/pack-v0.33.2-linux.tgz`, "Specifies the download url of the Pack Linux amd64 tar binary file. This can be found at https://github.com/buildpacks/pack/releases/.")
	...
	cmd.Flags().StringVar(&stepConfig.AgentLabel, "agentLabel", os.Getenv("PIPER_agentLabel"), "Agent label to run")
...
func packBuildMetadata() config.StepData {
			...
			Inputs: config.StepInputs{
				...
				Parameters: []config.StepParameters{
					...
					},
					{
						...
						Mandatory:   false,
						Aliases:     []config.Alias{},
						Default:     os.Getenv("PIPER_agentLabel"),
					},
				},
			},

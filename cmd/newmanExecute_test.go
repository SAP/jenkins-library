...
func TestRunNewmanExecute(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		assert.Contains(t, utils.executedScripts, executedScript{shell: "/bin/sh", script: "/home/node/.npm-global/bin/newman run localFile.json --reporters junit,html --reporter-junit-export target/newman/TEST-localFile.xml --reporter-html-export target/newman/TEST-localFile.html --suppress-exit-code"})
		assert.Contains(t, utils.executedScripts, executedScript{shell: "/bin/sh", script: "/home/node/.npm-global/bin/newman run localFile2.json --reporters junit,html --reporter-junit-export target/newman/TEST-localFile2.xml --reporter-html-export target/newman/TEST-localFile2.html --suppress-exit-code"})
		// assert.Contains(t, utils.executedScript, executedExecutables{executable: ".\\node_modules\\.bin\\newman", params: []string{"run", "localFile.json", "--environment", "", "--globals", "", "--reporters", "junit,html", "--reporter-junit-export", "target/newman/TEST-localFile.xml", "--reporter-html-export", "target/newman/TEST-localFile.html", "--suppress-exit-code"}})

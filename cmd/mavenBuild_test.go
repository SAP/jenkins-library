...
func TestMavenBuild(t *testing.T) {
	...
	t.Run("mavenBuild should create build artifacts metadata when CreateBuildArtifactsMetadata is true and Publish is true", func(t *testing.T) {
		mockedUtils := newMavenMockUtils()
		mockedUtils.AddFile("pom.xml", []byte{})
		config := mavenBuildOptions{CreateBuildArtifactsMetadata: true, Publish: true, AltDeploymentRepositoryURL: "http://sampleRepo.com"}
		err := runMavenBuild(&config, nil, &mockedUtils, &cpe)
		assert.Nil(t, err)
		assert.Equal(t, mockedUtils.Calls[0].Exec, "mvn")

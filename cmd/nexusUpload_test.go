...
func TestRunNexusUpload(t *testing.T) {
	...
	t.Run("uploading without any repos fails step", func(t *testing.T) {
		...
		err := runNexusUpload(utils, &uploader, &options)
		assert.EqualError(t, err, "none of the parameters 'mavenRepository' and 'npmRepository' are configured, or 'format' should be set if the 'url' already contain the repository ID")
	})

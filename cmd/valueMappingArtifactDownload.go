...
func valueMappingArtifactDownload(config valueMappingArtifactDownloadOptions, telemetryData *telemetry.CustomData) {
	...
	err := runValueMappingArtifactDownload(&config, telemetryData, httpClient)
	if err != nil {
		log.Entry().WithError(err).Fatal("step execution failed")
	}
}

func runValueMappingArtifactDownload(config *valueMappingArtifactDownloadOptions, telemetryData *telemetry.CustomData, httpClient piperhttp.Sender) error {
	...
	downloadArtifactURL := fmt.Sprintf("%s/api/v1/ValueMappingDesigntimeArtifacts(Id='%s',Version='%s')/$value", serviceKey.OAuth.Host, config.ValueMappingID, config.ValueMappingVersion)
	...
	return errors.Errorf("Value mapping artifact download failed, Response Status code: %v", downloadResp.StatusCode)
}

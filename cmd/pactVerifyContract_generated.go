...
func PactVerifyContractCommand() *cobra.Command {
	const STEP_NAME = "pactVerifyContract"

	metadata := pactVerifyContractMetadata()
	var stepConfig pactVerifyContractOptions
	var startTime time.Time
	var logCollector *log.CollectorHook
	var splunkClient *splunk.Splunk
	telemetryClient := &telemetry.Telemetry{}

	var createPactVerifyContractCmd = &cobra.Command{
		Use:   STEP_NAME,
		Short: "Step to execute PACT tests",
...
Run swagger-mock-validator to validate whether swagger fits pact test cases`,
		...
		Run: func(_ *cobra.Command, _ []string) {
			...
			pactVerifyContract(stepConfig, &stepTelemetryData)
			stepTelemetryData.ErrorCode = "0"
			log.Entry().Info("SUCCESS")
		},
	}

	addPactVerifyContractFlags(createPactVerifyContractCmd, &stepConfig)
	return createPactVerifyContractCmd
}

func addPactVerifyContractFlags(cmd *cobra.Command, stepConfig *pactVerifyContractOptions) {
	...
	cmd.Flags().StringVar(&stepConfig.Provider, "provider", os.Getenv("PIPER_provider"), "Provider calling PACT verification")
...
func pactVerifyContractMetadata() config.StepData {
	var theMetaData = config.StepData{
		Metadata: config.StepMetadata{
			Name:        "pactVerifyContract",
			Aliases:     []config.Alias{},
			Description: "Step to execute PACT tests",
		},
		Spec: config.StepSpec{
			Inputs: config.StepInputs{
				Parameters: []config.StepParameters{
					{
						Name:        "asynchApiFilePath",
						ResourceRef: []config.ResourceReference{},
						Scope:       []string{"PARAMETERS", "STAGES", "STEPS"},
						Type:        "string",
						Mandatory:   false,
						Aliases:     []config.Alias{},
						Default:     `api/build/swagger.json`,
					},
					{
						Name:        "buildId",
						ResourceRef: []config.ResourceReference{},

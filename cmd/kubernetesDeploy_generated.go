...
func addKubernetesDeployFlags(cmd *cobra.Command, stepConfig *kubernetesDeployOptions) {
	...
	cmd.Flags().StringVar(&stepConfig.ContainerName, "containerName", os.Getenv("PIPER_containerName"), "Only for `deployCommand: setImage` with a single image: Defines the name of the container in the Kubernetes deployment resource whose image should be updated. Used in `kubectl set image deployment/<deploymentName> <containerName>=<image>`. For multiple images, use `containerNames` instead.")

...
func TestParseATCResult(t *testing.T) {
	...
	t.Run("succes case: test parsing example XML result - Fail on Severity error", func(t *testing.T) {
		...
		expErr := errors.New("Step Execution failed due to at least one ATC Finding with severity equal (and higher) to configured failOnServerity Option - '" + doFailOnSeverityLevel + "'")

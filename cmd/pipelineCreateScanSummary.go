...
func runPipelineCreateScanSummary(config *pipelineCreateScanSummaryOptions, telemetryData *telemetry.CustomData, utils pipelineCreateScanSummaryUtils) error {
	...
	if len(config.PipelineLink) > 0 {
		output = []byte(fmt.Sprintf("## Pipeline Source for Details (since listed results might be incomplete)\n\nIt is crucial that you follow this link to check the detailed pipeline status: [%v](%v)\n\n", config.PipelineLink, config.PipelineLink))
	}

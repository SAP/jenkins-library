...
func addDetectExecuteScanFlags(cmd *cobra.Command, stepConfig *detectExecuteScanOptions) {
	...
	cmd.Flags().StringSliceVar(&stepConfig.CustomEnvironmentVariables, "customEnvironmentVariables", []string{`DETECT_LATEST_RELEASE_VERSION=6.8.0`}, "A list of environment variables which can be set to prepare the environment to run a BlackDuck scan. Specifically used to set the BlackDuck environment variables which control the scanning script")

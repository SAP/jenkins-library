...
func messageMappingDownload(config messageMappingDownloadOptions, telemetryData *telemetry.CustomData) {
	...
	err := runMessageMappingDownload(&config, telemetryData, httpClient)
	if err != nil {
		log.Entry().WithError(err).Fatal("step execution failed")
	}
}

func runMessageMappingDownload(config *messageMappingDownloadOptions, telemetryData *telemetry.CustomData, httpClient piperhttp.Sender) error {
	...
	downloadArtifactURL := fmt.Sprintf("%s/api/v1/MessageMappingDesigntimeArtifacts(Id='%s',Version='%s')/$value", serviceKey.OAuth.Host, config.MessageMappingID, config.MessageMappingVersion)
	...
	return errors.Errorf("Message mapping artifact download failed, Response Status code: %v", downloadResp.StatusCode)
}

...
func (p *buildahExecuteCommonPipelineEnvironment) persist(path, resourceName string) {
	content := []struct {
		category string
		name     string
		value    interface{}
...
type buildahExecuteReports struct {
}

func (p *buildahExecuteReports) persist(stepConfig buildahExecuteOptions, gcpJsonKeyFilePath string, gcsBucketId string, gcsFolderPath string, gcsSubFolder string) {
	if gcsBucketId == "" {
...
// BuildahExecuteCommand Executes a Buildah build for creating a container image.
func BuildahExecuteCommand() *cobra.Command {
	const STEP_NAME = "buildahExecute"

	metadata := buildahExecuteMetadata()
	var stepConfig buildahExecuteOptions
	var startTime time.Time
	var commonPipelineEnvironment buildahExecuteCommonPipelineEnvironment
	var reports buildahExecuteReports
	...
	var createBuildahExecuteCmd = &cobra.Command{
		Use:   STEP_NAME,
		Short: "Executes a Buildah build for creating a container image.",
		Long: `Executes a Buildah build for creating a container image.
Buildah is a tool that facilitates building Open Container Initiative (OCI) container images.`,
	...
	addBuildahExecuteFlags(createBuildahExecuteCmd, &stepConfig)
	return createBuildahExecuteCmd
}

func addBuildahExecuteFlags(cmd *cobra.Command, stepConfig *buildahExecuteOptions) {
	cmd.Flags().StringSliceVar(&stepConfig.BuildOptions, "buildOptions", []string{``}, "Defines a list of build options for buildah")
...
func buildahExecuteMetadata() config.StepData {

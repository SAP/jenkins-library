...
func CreateGeneralConfigCopyFromFlags(cmd *cobra.Command) (GeneralConfigOptions, error) {
	gc := GeneralConfigOptions{}
	var err error

	//All fields of structure are either read from flags or treaded below
	//GitHubAccessTokens done below (func resolveAccessTokens)
	if gc.CorrelationID, err = cmd.Flags().GetString("correlationID"); err != nil {
		return gc, err
	}
	if gc.CustomConfig, err = cmd.Flags().GetString("customConfig"); err != nil {
		return gc, err
	}
	if gc.GitHubTokens, err = cmd.Flags().GetStringSlice("gitHubTokens"); err != nil {
		return gc, err
	}
	if gc.DefaultConfig, err = cmd.Flags().GetStringSlice("defaultConfig"); err != nil {
		return gc, err
	}
	if gc.IgnoreCustomDefaults, err = cmd.Flags().GetBool("ignoreCustomDefaults"); err != nil {
		return gc, err
	}
	if gc.ParametersJSON, err = cmd.Flags().GetString("parametersJSON"); err != nil {
		return gc, err
	}
	if gc.EnvRootPath, err = cmd.Flags().GetString("envRootPath"); err != nil {
		return gc, err
	}
	if gc.NoTelemetry, err = cmd.Flags().GetBool("noTelemetry"); err != nil {
		return gc, err
	}
	if gc.StageName, err = cmd.Flags().GetString("stageName"); err != nil {
		return gc, err
	}
	if gc.StepConfigJSON, err = cmd.Flags().GetString("stepConfigJSON"); err != nil {
		return gc, err
	}
	//StepMetadata
	if gc.Verbose, err = cmd.Flags().GetBool("verbose"); err != nil {
		return gc, err
	}
	if gc.LogFormat, err = cmd.Flags().GetString("logFormat"); err != nil {
		return gc, err
	}
	//VaultRoleID -> prepareConfig
	//VaultRoleSecretID -> prepareConfig
	//VaultToken -> prepareConfig
	if gc.VaultServerURL, err = cmd.Flags().GetString("vaultServerUrl"); err != nil {
		return gc, err
	}
	if gc.VaultNamespace, err = cmd.Flags().GetString("vaultNamespace"); err != nil {
		return gc, err
	}
	if gc.VaultPath, err = cmd.Flags().GetString("vaultPath"); err != nil {
		return gc, err
	}
	//HookConfig -> prepareconfig -> retrieveHookConfig
	//MetaDataResolver
	if gc.GCPJsonKeyFilePath, err = cmd.Flags().GetString("gcpJsonKeyFilePath"); err != nil {
		return gc, err
	}
	if gc.GCSFolderPath, err = cmd.Flags().GetString("gcsFolderPath"); err != nil {
		return gc, err
	}
	if gc.GCSBucketId, err = cmd.Flags().GetString("gcsBucketId"); err != nil {
		return gc, err
	}
	if gc.GCSSubFolder, err = cmd.Flags().GetString("gcsSubFolder"); err != nil {
		return gc, err
	}
	if gc.GitCommit, err = cmd.Flags().GetString("gitCommit"); err != nil {
		return gc, err
	}

	gc.GitHubAccessTokens = resolveAccessTokens(gc.GitHubTokens)
...
func (GeneralConfig *GeneralConfigOptions) PrepareConfig(cmd *cobra.Command, metadata *config.StepData, stepName string, options interface{}, openFile func(s string, t map[string]string) (io.ReadCloser, error)) error {

	log.SetFormatter(GeneralConfig.LogFormat)

	GeneralConfig.initStageName(true)
...
func (GeneralConfig *GeneralConfigOptions) initStageName(outputToLog bool) {

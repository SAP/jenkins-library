...
func addGradleExecuteBuildFlags(cmd *cobra.Command, stepConfig *gradleExecuteBuildOptions) {
	...
	cmd.Flags().BoolVar(&stepConfig.UseArtifactoryMirror, "useArtifactoryMirror", false, "If set to true, an init script is injected that adds the SAP Artifactory repository to all projects' dependency and plugin classpaths. This prepends Artifactory to the resolution order, so dependencies available there are fetched from Artifactory. Any artifact not found in Artifactory will still fall back to the repositories declared in the project's own build scripts (e.g. Maven Central).")

...
func scriptCollectionDownload(config scriptCollectionDownloadOptions, telemetryData *telemetry.CustomData) {
	...
	err := runScriptCollectionDownload(&config, telemetryData, httpClient)
	if err != nil {
		log.Entry().WithError(err).Fatal("step execution failed")
	}
}

func runScriptCollectionDownload(config *scriptCollectionDownloadOptions, telemetryData *telemetry.CustomData, httpClient piperhttp.Sender) error {
	...
	downloadArtifactURL := fmt.Sprintf("%s/api/v1/ScriptCollectionDesigntimeArtifacts(Id='%s',Version='%s')/$value", serviceKey.OAuth.Host, config.ScriptCollectionID, config.ScriptCollectionVersion)
	...
	return errors.Errorf("Script Collection artifact download failed, Response Status code: %v", downloadResp.StatusCode)
}

...
func checkIfStepActive(utils piperutils.FileUtils) error {
	...
	if currentOrchestrator == "Jenkins" {
		...
		log.Entry().Info("CBfix [Central Build] config: ", runConfigV1.RunConfig.StageConfig.Stages["Central Build"])
		log.Entry().Info("CBfix [Build] config: ", runConfigV1.RunConfig.StageConfig.Stages["Build"])

...
func generateManifestYAML(config abapEnvironmentCreateSystemOptions) ([]byte, error) {
	...
	config, err := checkAbapOemSystem(&config)
	if err != nil {
...
func checkAbapOemSystem(config *abapEnvironmentCreateSystemOptions) (options abapEnvironmentCreateSystemOptions, err error) {

	if config.CfService == "abap-oem" {
		const parentSaaSAppname = "addon_test"
		config.AbapSystemParentSaaSAppname = parentSaaSAppname
	}
	options = *config

...
func (cx1sh *checkmarxoneExecuteScanHelper) RunScan(ctx context.Context, config checkmarxoneExecuteScanOptions, sys checkmarxone.System, influx *checkmarxoneExecuteScanInflux, utils checkmarxoneExecuteScanUtils) error {
	...
	if len(projects) == 0 {
		if len(groupID) == 0 {
			return errors.Wrap(err, "GroupName or GroupID is required to create a new project")
		}

		log.Entry().WithError(err).Error("Project not found - creating?")
		return errors.Wrap(err, "No project found")
	}
...
func (cx1sh *checkmarxoneExecuteScanHelper) createNewProject(config checkmarxoneExecuteScanOptions, sys checkmarxone.System, projectName string, groupIDs []string) (checkmarxone.Project, error) {
	log.Entry().Infof("Project %v does not exist, starting to create it...", projectName)
	//presetID, _ := strconv.Atoi(config.Preset)
	project, err := cx1sh.createAndConfigureNewProject(sys, projectName, groupIDs, config.Preset, config.SourceEncoding)
	if err != nil {
		return checkmarxone.Project{}, errors.Wrapf(err, "failed to create and configure new project %v", projectName)
	}
	return project, nil
}
...
func (cx1sh *checkmarxoneExecuteScanHelper) uploadAndScan(ctx context.Context, config checkmarxoneExecuteScanOptions, sys checkmarxone.System, project checkmarxone.Project, influx *checkmarxoneExecuteScanInflux, utils checkmarxoneExecuteScanUtils) error {
	previousScans, err := sys.GetScans(project.ProjectID)
	...
	if len(previousScans) > 0 && config.VerifyOnly {
		...
		return cx1sh.triggerScan(ctx, config, sys, project, uploadUri, "zip", incremental, influx, utils)
	}
	return nil
}

func (cx1sh *checkmarxoneExecuteScanHelper) triggerScan(ctx context.Context, config checkmarxoneExecuteScanOptions, sys checkmarxone.System, project checkmarxone.Project, repoUrl string, branch string, incremental bool, influx *checkmarxoneExecuteScanInflux, utils checkmarxoneExecuteScanUtils) error {
...
func (cx1sh *checkmarxoneExecuteScanHelper) pollScanStatus(sys checkmarxone.System, scan checkmarxone.Scan) error {
	...
	stageDetail := "..."
	for {
		var detail checkmarxone.ScanStatusDetail
	...
	if status == "Failed" {
		...
		return fmt.Errorf("Checkmarx One scan failed with the following error: %v", stageDetail)
	} */
	return nil
}
...
func (cx1sh *checkmarxoneExecuteScanHelper) createAndConfigureNewProject(sys checkmarxone.System, projectName string, groupIDs []string, presetValue, engineConfiguration string) (checkmarxone.Project, error) {
	if len(presetValue) == 0 {
		log.SetErrorCategory(log.ErrorConfiguration)
		return checkmarxone.Project{}, fmt.Errorf("preset not specified, creation of project %v failed", projectName)
	}

	project, err := sys.CreateProject(projectName, groupIDs)
	if err != nil {

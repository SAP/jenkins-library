...
import (
	...
	cmdPkg "github.com/SAP/jenkins-library/cmd"
...
func CnbBuildCommand() *cobra.Command {
	...
	metadata := metadata.CnbBuildMetadata()
...
**Important:** Please note, that the cnbBuild step is in **beta** state, and there could be breaking changes before we remove the beta notice.`,
		PreRunE: func(cmd *cobra.Command, _ []string) error {
			...
			log.SetVerbose(cmdPkg.GeneralConfig.Verbose)

			cmdPkg.GeneralConfig.GitHubAccessTokens = cmdPkg.ResolveAccessTokens(cmdPkg.GeneralConfig.GitHubTokens)

			path, _ := os.Getwd()
			fatalHook := &log.FatalHook{CorrelationID: cmdPkg.GeneralConfig.CorrelationID, Path: path}
			log.RegisterHook(fatalHook)

			err := cmdPkg.PrepareConfig(cmd, &metadata, STEP_NAME, &stepConfig, config.OpenPiperFile)
			if err != nil {
			...
			if len(cmdPkg.GeneralConfig.HookConfig.SentryConfig.Dsn) > 0 {
				sentryHook := log.NewSentryHook(cmdPkg.GeneralConfig.HookConfig.SentryConfig.Dsn, cmdPkg.GeneralConfig.CorrelationID)
				log.RegisterHook(&sentryHook)
			}

			if len(cmdPkg.GeneralConfig.HookConfig.SplunkConfig.Dsn) > 0 || len(cmdPkg.GeneralConfig.HookConfig.SplunkConfig.ProdCriblEndpoint) > 0 {
				splunkClient = &splunk.Splunk{}
				logCollector = &log.CollectorHook{CorrelationID: cmdPkg.GeneralConfig.CorrelationID}
				log.RegisterHook(logCollector)
			}

			if err = log.RegisterANSHookIfConfigured(cmdPkg.GeneralConfig.CorrelationID); err != nil {
		...
		Run: func(_ *cobra.Command, _ []string) {
			...
			handler := func() {
				commonPipelineEnvironment.persist(cmdPkg.GeneralConfig.EnvRootPath, "commonPipelineEnvironment")
				reports.persist(stepConfig,cmdPkg.GeneralConfig.GCPJsonKeyFilePath,cmdPkg.GeneralConfig.GCSBucketId,cmdPkg.GeneralConfig.GCSFolderPath,cmdPkg.GeneralConfig.GCSSubFolder)
				config.RemoveVaultSecretFiles()
				stepTelemetryData.Duration = fmt.Sprintf("%v", time.Since(startTime).Milliseconds())
				stepTelemetryData.ErrorCategory = log.GetErrorCategory().String()
				stepTelemetryData.PiperCommitHash = cmdPkg.GitCommit
				telemetryClient.SetData(&stepTelemetryData)
				telemetryClient.Send()
				if len(cmdPkg.GeneralConfig.HookConfig.SplunkConfig.Dsn) > 0 {
					splunkClient.Initialize(cmdPkg.GeneralConfig.CorrelationID,
					cmdPkg.GeneralConfig.HookConfig.SplunkConfig.Dsn,
					cmdPkg.GeneralConfig.HookConfig.SplunkConfig.Token,
					cmdPkg.GeneralConfig.HookConfig.SplunkConfig.Index,
					cmdPkg.GeneralConfig.HookConfig.SplunkConfig.SendLogs)
					splunkClient.Send(telemetryClient.GetData(), logCollector)
				}
				if len(cmdPkg.GeneralConfig.HookConfig.SplunkConfig.ProdCriblEndpoint) > 0 {
					splunkClient.Initialize(cmdPkg.GeneralConfig.CorrelationID,
					cmdPkg.GeneralConfig.HookConfig.SplunkConfig.ProdCriblEndpoint,
					cmdPkg.GeneralConfig.HookConfig.SplunkConfig.ProdCriblToken,
					cmdPkg.GeneralConfig.HookConfig.SplunkConfig.ProdCriblIndex,
					cmdPkg.GeneralConfig.HookConfig.SplunkConfig.SendLogs)
					splunkClient.Send(telemetryClient.GetData(), logCollector)
				}
			}
			...
			defer handler()
			telemetryClient.Initialize(cmdPkg.GeneralConfig.NoTelemetry, STEP_NAME)

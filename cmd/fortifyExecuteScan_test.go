...
func TestAutoresolveClasspath(t *testing.T) {
	...
	t.Run("error pip file", func(t *testing.T) {
		...
		_, err := autoresolvePipClasspath("python2", []string{"-c", "import sys;p=sys.path;p.remove('');print(';'.join(p))"}, "/dev/null", &utils)
		assert.Error(t, err)
	})
	...
	t.Run("error maven", func(t *testing.T) {
		...
		_, err := autoresolveMavenClasspath(fortifyExecuteScanOptions{BuildDescriptorFile: "pom.xml"}, "/dev/null", &utils)
		assert.Error(t, err)
	})
}

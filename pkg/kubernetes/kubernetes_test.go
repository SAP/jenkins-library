...
func TestRunHelmDeploy(t *testing.T) {
	...
	t.Run("test helm", func(t *testing.T) {
		...
		telemetryData := &telemetry.CustomData{
			...
			Custom1:      opts.DeployTool,
		}
		kubernetesDeploy := KubernetesDeployBundle{
			config: opts,
			utils:  mockUtils,
			stdout: &stdout,
		}
		kubernetesDeploy.RunHelmDeploy()
	...
	t.Run("test helm - docker config.json path passed as parameter", func(t *testing.T) {
		...
		mockUtils.StdoutReturn = map[string]string{
			`kubectl create secret generic testSecret --from-file=.dockerconfigjson=/path/to/.docker/config.json --type=kubernetes.io/dockerconfigjson --insecure-skip-tls-verify=true --dry-run=client --output=json`: dockerConfigJSON,
		}
		...
		err := kubernetesDeploy.RunHelmDeploy()
	...
	t.Run("test helm v3 - with multiple images", func(t *testing.T) {
		...
		require.NoError(t, kubernetesDeploy.RunHelmDeploy())
	...
	t.Run("test helm v3 - with multiple images and incorrect valuesMapping", func(t *testing.T) {
		...
		require.Error(t, kubernetesDeploy.RunHelmDeploy(), "invalid path 'false' is used for valueMapping, only strings are supported")
	})
...
func TestRunKubctlDeploy(t *testing.T) {

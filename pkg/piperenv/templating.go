...
func (c *CPEMap) cpe(element string) string {
	...
	return fmt.Sprint(map[string]any(*c)[element])
}

func (c *CPEMap) custom(element string) string {
	return fmt.Sprint(map[string]any(*c)[fmt.Sprintf("custom/%v", element)])
}

func (c *CPEMap) git(element string) string {
	var el string
	if element == "organization" || element == "repository" {
		el = fmt.Sprint(map[string]any(*c)[fmt.Sprintf("github/%v", element)])
	} else {
		el = fmt.Sprint(map[string]any(*c)[fmt.Sprintf("git/%v", element)])
	}
	return el
}

func (c *CPEMap) imageDigest(imageName string) string {
	digests, _ := map[string]any(*c)["container/imageDigests"].([]any)
	imageNames, _ := map[string]any(*c)["container/imageNames"].([]any)
...
func (c *CPEMap) imageTag(imageName string) string {
	nameTags, _ := map[string]any(*c)["container/imageNameTags"].([]any)

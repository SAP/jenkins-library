...
func (c *CPEMap) ParseTemplate(cpeTemplate string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to parse cpe template '%v': %w", cpeTemplate, err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("failed to execute cpe template '%v': %w", cpeTemplate, err)
	}

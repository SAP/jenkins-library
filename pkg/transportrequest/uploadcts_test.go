		...
		t.Run("all possible values provided", func(t *testing.T) {
			...
			if assert.NoError(t, err) {
				assert.Contains(
					t,
					cmd.Calls[0],
					"fiori deploy -f -y --username ABAP_USER --password ABAP_PASSWORD -e \"the Desc\" --noConfig --url https://example.org:8080/cts -l 001 -t 12345678 -p abapPackage -n appName",
				)

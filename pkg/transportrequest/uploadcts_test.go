		...
		t.Run("all possible values provided", func(t *testing.T) {
			...
			if assert.NoError(t, err) {
				assert.Contains(
					t,
					cmd.Calls[0],
					"fiori deploy -f -y --username ABAP_USER --password ABAP_PASSWORD -e \"the Desc\" --noConfig --url https://example.org:8080/cts --client 001 --transport 12345678 --package abapPackage --name appName",
				)
		...
		t.Run("all possible values omitted", func(t *testing.T) {
			...
			if assert.NoError(t, err) {
				assert.Contains(
					t,
					cmd.Calls[0],
					"fiori deploy -f -y --username ABAP_USER --password ABAP_PASSWORD -e \"Deployed with Piper based on SAP Fiori tools\" --noConfig --transport 12345678",
				)
				assert.Equal(t, []string{"ABAP_USER=me", "ABAP_PASSWORD=******"}, cmd.Env)
			}
		})

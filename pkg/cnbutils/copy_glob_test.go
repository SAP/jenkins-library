...
func TestCopyGlob(t *testing.T) {
	t.Run("copies file according to doublestart glob", func(t *testing.T) {
		...
		err := cnbutils.CopyGlob("/workdir/src", "/dest", "**/*.yaml", mockUtils)
	...
	t.Run("copies file according to simple globs", func(t *testing.T) {
		...
		err := cnbutils.CopyGlob("/src", "/dest", "*.yaml", mockUtils)
		assert.NoError(t, err)

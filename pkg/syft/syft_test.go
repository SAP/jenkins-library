...
func TestGenerateSBOM(t *testing.T) {
	...
	t.Run("should generate SBOM", func(t *testing.T) {
		...
		assert.Equal(t, firstCall.Params, []string{"packages", "registry:my-registry/image:latest", "--output", "cyclonedx-xml=bom-docker-0.xml", "-q"})

		secondCall := execMock.Calls[1]
		assert.Equal(t, secondCall.Exec, "/tmp/syfttest/syft")
		assert.Equal(t, secondCall.Params, []string{"packages", "registry:my-registry/image:1.2.3", "--output", "cyclonedx-xml=bom-docker-1.xml", "-q"})
	})

	t.Run("error case: syft execution failed", func(t *testing.T) {
		execMock = mock.ExecMockRunner{}
		execMock.ShouldFailOnCommand = map[string]error{
			"/tmp/syfttest/syft packages registry:my-registry/image:latest --output cyclonedx-xml=bom-docker-0.xml -q": errors.New("failed"),
		}

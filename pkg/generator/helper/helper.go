...
type stepInfo struct {
	...
	AuthAlternatives  []config.AuthAlternative
...
const (
	metadataGeneratedFileName = "metadata_generated.go"
	metadataGeneratedTemplate = `// Code generated by piper's step-generator. DO NOT EDIT.
...
func ProcessMetaFiles(metadataFiles []string, targetDir string, stepHelperData StepHelperData) error {
	...
	for key := range metadataFiles {
		...
		if err = stepHelperData.WriteFile(filepath.Join(targetDir, fmt.Sprintf("%v_generated.go", stepName)), step, 0o644); err != nil {
			log.Fatalf("Error occurred: %v\n", err)
		}

		test := stepTemplate(myStepInfo, "stepTest", stepTestGoTemplate)
		if err = stepHelperData.WriteFile(filepath.Join(targetDir, fmt.Sprintf("%v_generated_test.go", stepName)), test, 0o644); err != nil {
			log.Fatalf("Error occurred: %v\n", err)
		}
		...
		if !exists {
			...
			if err = stepHelperData.WriteFile(filepath.Join(targetDir, fmt.Sprintf("%v.go", stepName)), impl, 0o644); err != nil {
				log.Fatalf("Error occurred: %v\n", err)
			}
		}
		...
		if !exists {
			...
			if err = stepHelperData.WriteFile(filepath.Join(targetDir, fmt.Sprintf("%v_test.go", stepName)), impl, 0o644); err != nil {
				log.Fatalf("Error occurred: %v\n", err)
			}
		}
	}
	...
	if err := stepHelperData.WriteFile(filepath.Join(targetDir, metadataGeneratedFileName), code, 0o644); err != nil {
		log.Fatalf("Error occurred: %v\n", err)
	}
...
func getStepInfo(stepData *config.StepData, exportPrefix string) (stepInfo, error) {
	...
	return stepInfo{
			...
			AuthAlternatives:  stepData.Spec.Inputs.AuthAlternatives,

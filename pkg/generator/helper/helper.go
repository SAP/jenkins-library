...
func {{.CobraCmdFuncName}}() *cobra.Command {
	...
	metadata := metadata.{{ .StepName | title }}Metadata()
...
const stepMetadataGoTemplate = `// Code generated by piper's step-generator. DO NOT EDIT.
...
func {{ .StepName | title }}Metadata() config.StepData {
...
const stepGoImplementationTemplate = `package {{ .Package }}
import (
	"fmt"
...
const stepGoImplementationTestTemplate = `package {{ .Package }}
...
func GetAllStepMetadata() map[string]config.StepData {
	return map[string]config.StepData{
		{{range $stepName := .Steps }} {{ $stepName | quote }}: {{$stepName | title}}Metadata(),
		{{end}}
	}
}
`
...
func ProcessMetaFiles(metadataFiles []string, targetDir string, stepHelperData StepHelperData) error {
	...
	for key := range metadataFiles {
		...
		pkgDir := targetDir
		exportPrefix := ""
		if stepData.Metadata.Package != "" {
			pkgDir = path.Join(pkgDir, stepData.Metadata.Package)
		...
		myStepInfo, err := getStepInfo(&stepData, osImport, exportPrefix)
		checkError(err)

		step := stepTemplate(myStepInfo, "step", stepGoTemplate)
		err = stepHelperData.WriteFile(filepath.Join(pkgDir, fmt.Sprintf("%v_generated.go", stepName)), step, 0644)
		checkError(err)

		stepMetadata := stepTemplate(myStepInfo, "stepMetadata", stepMetadataGoTemplate)
		err = stepHelperData.WriteFile(filepath.Join(targetDir, "metadata", fmt.Sprintf("%v_generated.go", stepName)), stepMetadata, 0644)
		checkError(err)

		test := stepTemplate(myStepInfo, "stepTest", stepTestGoTemplate)
		err = stepHelperData.WriteFile(filepath.Join(pkgDir, fmt.Sprintf("%v_generated_test.go", stepName)), test, 0644)
		checkError(err)

		exists, _ := piperutils.FileExists(filepath.Join(pkgDir, fmt.Sprintf("%v.go", stepName)))
		if !exists {
			impl := stepImplementation(myStepInfo, "impl", stepGoImplementationTemplate)
			err = stepHelperData.WriteFile(filepath.Join(pkgDir, fmt.Sprintf("%v.go", stepName)), impl, 0644)
			checkError(err)
		}

		exists, _ = piperutils.FileExists(filepath.Join(pkgDir, fmt.Sprintf("%v_test.go", stepName)))
		if !exists {
			impl := stepImplementation(myStepInfo, "implTest", stepGoImplementationTestTemplate)
			err = stepHelperData.WriteFile(filepath.Join(pkgDir, fmt.Sprintf("%v_test.go", stepName)), impl, 0644)
			checkError(err)
		}
	}
	...
	err := stepHelperData.WriteFile(filepath.Join(targetDir, "metadata", metadataGeneratedFileName), code, 0644)
	checkError(err)
...
func getStepInfo(stepData *config.StepData, osImport bool, exportPrefix string) (stepInfo, error) {
	...
	pack := "cmd"
	if stepData.Metadata.Package != "" {
		pack = stepData.Metadata.Package
	}
	return stepInfo{
			StepName:         stepData.Metadata.Name,

...
func ProcessMetaFiles(metadataFiles []string, targetDir string, stepHelperData StepHelperData) error {
	...
	for key := range metadataFiles {
		...
		stepHelperData.FileUtils.MkdirAll(filepath.Join(targetDir, myStepInfo.Package), 0644)

		step := stepTemplate(myStepInfo, "step", stepGoTemplate)
		err = stepHelperData.WriteFile(filepath.Join(targetDir, myStepInfo.Package, fmt.Sprintf("%v_generated.go", stepName)), step, 0644)
		checkError(err)

		test := stepTemplate(myStepInfo, "stepTest", stepTestGoTemplate)
		err = stepHelperData.WriteFile(filepath.Join(targetDir, myStepInfo.Package, fmt.Sprintf("%v_generated_test.go", stepName)), test, 0644)
		checkError(err)

		exists, _ := piperutils.FileExists(filepath.Join(targetDir, myStepInfo.Package, fmt.Sprintf("%v.go", stepName)))
		if !exists {
			impl := stepImplementation(myStepInfo, "impl", stepGoImplementationTemplate)
			err = stepHelperData.WriteFile(filepath.Join(targetDir, myStepInfo.Package, fmt.Sprintf("%v.go", stepName)), impl, 0644)
			checkError(err)
		}

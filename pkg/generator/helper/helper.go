...
func {{.CobraCmdFuncName}}() *cobra.Command {
	...
	metadata := {{ .StepNameTitle }}Metadata()
...
func {{ .StepNameTitle }}Metadata() config.StepData {
...
{{range $package := .Packages }}import "github.com/SAP/jenkins-library/cmd/{{$package}}"
{{end}}
...
func ProcessMetaFiles(metadataFiles []string, targetDir string, stepHelperData StepHelperData) error {
	...
	for key := range metadataFiles {
		...
		stepHelperData.FileUtils.MkdirAll(filepath.Join(targetDir, stepData.Metadata.Package), 0644)

		step := stepTemplate(myStepInfo, "step", stepGoTemplate)
		err = stepHelperData.WriteFile(filepath.Join(targetDir, stepData.Metadata.Package, fmt.Sprintf("%v_generated.go", stepName)), step, 0644)
		checkError(err)

		test := stepTemplate(myStepInfo, "stepTest", stepTestGoTemplate)
		err = stepHelperData.WriteFile(filepath.Join(targetDir, stepData.Metadata.Package, fmt.Sprintf("%v_generated_test.go", stepName)), test, 0644)
		checkError(err)

		exists, _ := piperutils.FileExists(filepath.Join(targetDir, stepData.Metadata.Package, fmt.Sprintf("%v.go", stepName)))
		if !exists {
			impl := stepImplementation(myStepInfo, "impl", stepGoImplementationTemplate)
			err = stepHelperData.WriteFile(filepath.Join(targetDir, stepData.Metadata.Package, fmt.Sprintf("%v.go", stepName)), impl, 0644)
			checkError(err)
		}

		exists, _ = piperutils.FileExists(filepath.Join(targetDir, stepData.Metadata.Package, fmt.Sprintf("%v_test.go", stepName)))
		if !exists {
			impl := stepImplementation(myStepInfo, "implTest", stepGoImplementationTestTemplate)
			err = stepHelperData.WriteFile(filepath.Join(targetDir, stepData.Metadata.Package, fmt.Sprintf("%v_test.go", stepName)), impl, 0644)
			checkError(err)
		}
	}
...
func getStepInfo(stepData *config.StepData, osImport bool, exportPrefix string) (stepInfo, error) {
	...
	stepNameTitle := piperutils.Title(stepData.Metadata.Name)

	return stepInfo{
			StepName:         stepData.Metadata.Name,
			CobraCmdFuncName: fmt.Sprintf("%vCommand", stepNameTitle),
			CreateCmdVar:     fmt.Sprintf("create%vCmd", stepNameTitle),
			...
			FlagsFunc:        fmt.Sprintf("add%vFlags", stepNameTitle),
			OSImport:         osImport,
			...
			Package:          stepData.Metadata.Package,
			StepNameTitle:    stepNameTitle,
		},
		err
}

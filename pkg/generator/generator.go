...
func ProcessMetaFiles(metadataFiles []string, targetDir string, stepHelperData StepHelperData) error {
	...
	code := metadataTemplate(allSteps)
...
func stepTemplate(myStepInfo stepInfo) []byte {
	...
	return generateCodeFromTemplate(myStepInfo, "templates/step.go.tmpl", funcMap)
}
...
func stepTestTemplate(myStepInfo stepInfo) []byte {
	...
	return generateCodeFromTemplate(myStepInfo, "templates/step_test.go.tmpl", funcMap)
}
...
func stepImplementation(myStepInfo stepInfo) []byte {
	...
	return generateCodeFromTemplate(myStepInfo, "templates/step_impl.go.tmpl", funcMap)
}
...
func stepImplementationTest(myStepInfo stepInfo) []byte {
	...
	return generateCodeFromTemplate(myStepInfo, "templates/step_impl_test.go.tmpl", funcMap)
}

func metadataTemplate(allSteps interface{}) []byte {
	return generateCodeFromTemplate(allSteps, "templates/metadata.go.tmpl", sprig.HermeticTxtFuncMap())
}

func generateCodeFromTemplate(dataObject interface{}, templatePath string, funcMap template.FuncMap) []byte {

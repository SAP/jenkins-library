...
if [[ ${REPOSITORY_UNDER_TEST} == SAP/jenkins-library ]]
then
    BRANCH_NAME=consumer-test
fi
...
docker run -v /var/run/docker.sock:/var/run/docker.sock -v "${PWD}":/workspace -v /tmp -e CASC_JENKINS_CONFIG=/workspace/jenkins.yml -e CX_INFRA_IT_CF_USERNAME -e CX_INFRA_IT_CF_PASSWORD -e BRANCH_NAME ppiper/jenkinsfile-runner
